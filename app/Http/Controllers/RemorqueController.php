...
use App\Http\Requests\Step1RRequest;
...
class RemorqueController extends Controller
{
    ...
    {
        $autos = Remorque::where('validation', 1)->where('enpublic',1)
            ->orderBy('created_at', 'desc')
            ->paginate(3);
        $new = Remorque::take('2')->where('validation',1)->orderBy('created_at' , 'desc')

            ->paginate(2);

        $timezone = date("Y/m/d");
        $Img = $Img->all()->where('ok',1);
        $newremorque = Remorque::take('2')->where('magazine',1)->orderBy('created_at' , 'desc') ->paginate(2);




        return view('front.autos.remorque.index', compact('autos','new' ,'Img','timezone','newremorque'));
    }



    public function get_remorque_model(Request $r)
    {
        if($r->ajax())
        {
            $select= $r->has('select') ? $r->input('select'):'';

            if($r->has('remorque_marque')){
                  return   Form::select('remorque_model',
                        AutoModele::where('remorque_marque_id',$r->input('remorque_marque'))->pluck('remorques_model','id'),$select,
                        ['class'=>'form-control m-selectt','placeholder'=>'Choose Model']);



            }
            elseif($r->has('remorque_marque_2'))
            {
                return   Form::select('remorque_model',
                    AutoModele::where('remorque_marque_id',$r->input('remorque_marque_2'))->pluck('remorques_model','id'),$select,
                    ['id'=>'select1','placeholder'=>'Choose Model']);
            }
            elseif($r->has('remorque_marque_3'))
            {
                return   Form::select('remorque_model',
                    AutoModele::where('remorque_marque_id',$r->input('remorque_marque_3'))->pluck('remorques_model','id'),$select,
    ...
    public function info1()
    {
        ...
        return view('front.autos.remorque.add');
    }
    // public function getinfo1(Step1RRequest $r)
    public function getinfo1(Request $r)
    {
        $r->session()->put('step1', $r->except('_token'));
        return \redirect('/remoroquestep2');
//
    }
    ...
    public function info2(Request $r)
    {
        ...
        if ($r->session()->has('step2')) {
            ...
            return view('front.autos.remorque.add')->with('input', $data);
        }
        if (!$r->session()->has('step1')) {
            return \redirect('/sellremorque');
        }
        return view('front.autos.remorque.add');
    }
    public function getinfo2(Request $r)
    {
        ...
        return \redirect('/remoroquestep4');
   ...
   public function getinfo4(Request $r)
    {
        ...
        return \redirect('/remoroquestep3');
    ...
    public function storeAuto(Request $autoRequest,Remorque $auto)
    {
    ...
    public function detailleremorque($id , Remorque $auto , RemorqueImages $images)
    {
        $autoInfo = $auto->findOrFail($id);

        return view('front.autos.remorque.detaille' , compact('autoInfo','autoImg' ));
    }

   public function searche(Request $request, Remorque $auto,AutoMarque $marque ,AutoModele $modele,PubH $pubh,PubV $pubv)
    {

        $requestAll = array_except($request->toArray(), ['submit', '_token', 'page']);
       $query = DB::table('remorques')->select('*');
        ...
        $selection = Remorque::where('validation', 1)->where('enpublic',1)
            ->orderBy('created_at', 'desc')
            ->paginate(3);
       ...
       $autos =  $query ->paginate(15);
        ...
        return view('front.autos.remorque.liste-annonces', compact('autos', 'array','pubh','pubv','selection'));
    ...
    public function advencedsearchliste()
    {
        ...
        $autos = Remorque::where('validation', 1)
            ->orderBy('id', 'desc')
            ->paginate(3);
        return view('front.autos.remorque.advencedsearch', compact('autos'));
    }
