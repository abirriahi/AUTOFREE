...
use App\Http\Requests\Step1CRequest;
...
class CamionController extends Controller
{
    ...
    {
        $autos = Camion::where('validation', 1)->where('enpublic',1)
            ->orderBy('created_at', 'desc')
            ->paginate(3);
        $new = Camion::take('2')->where('validation',1)->orderBy('created_at' , 'desc')
        ...
        return view('front.autos.index', compact('autos','new','newcamion' ,'Img','timezone'));
    }
   public function get_camion_model(Request $r)
    {
        if($r->ajax())
        {
            $select= $r->has('select') ? $r->input('select'):'';

            if($r->has('camion_marque')){
                  return   Form::select('camion_model',
                        AutoModele::where('camion_marque_id',$r->input('camion_marque'))->pluck('camions_model','id'),
                      $select,
            ...
            elseif($r->has('camion_marque_2'))
            {
                return   Form::select('camion_model',
                    AutoModele::where('camion_marque_id',$r->input('camion_marque_2'))->pluck('camions_model','id'),$select,
                    ['id'=>'select1','placeholder'=>'Choose Model']);
            }
            elseif($r->has('camion_marque_3'))
            {
                return   Form::select('camion_model',
                    AutoModele::where('camion_marque_id',$r->input('camion_marque_3'))->pluck('camions_model','id'),$select,
    ...
    public function info1()
    {
        ...
        return view('front.autos.camion.add');
    }
    // public function getinfo1(Step1CRequest $r)
    public function getinfo1(Request $r)
    {
        $r->session()->put('step1', $r->except('_token'));
        return \redirect('/camionstep2');
//
    }
    ...
    public function info2(Request $r)
    {
        ...
        if ($r->session()->has('step2')) {
            ...
            return view('front.autos.camion.add')->with('input', $data);
        }
        if (!$r->session()->has('step1')) {
            return \redirect('/sellcamion');
        }
        return view('front.autos.camion.add');
    }
...
//public function getinfo4(Step4Request $r)
   ...
   // public function storeAuto(AutoRequest $autoRequest, Auto $auto)
    public function storeAuto(Request $autoRequest, Camion $auto)
    {
     ...
     public function detaillecamion($id , Camion $auto ,CampingImages $images)
    {
        $autoInfo = $auto->findOrFail($id);

        return view('front.autos.camion.detaille' , compact('autoInfo','autoImg' ));
    }

   public function searche(Request $request, Camion $auto,AutoMarque $marque ,AutoModele $modele,PubH $pubh,PubV $pubv)
    {

        $requestAll = array_except($request->toArray(), ['submit', '_token', 'page']);
        $query = DB::table('camions')->select('*');
        ...
        $selection = Camion::where('validation', 1)->where('enpublic',1)
            ->orderBy('created_at', 'desc')
            ->paginate(3);
       ...
       $autos =  $query ->paginate(15);
        ...
        return view('front.autos.camion.liste-annonces', compact('autos', 'array','pubh', 'pubv','selection'));
    ...
    public function advencedsearchliste()
    {
        ...
        $autos = Camion::where('validation', 0)
            ->orderBy('id', 'desc')
            ->paginate(3);
        return view('front.autos.camion.advencedsearch', compact('autos'));
    }
