...
@section('content')
    ...
    <div class="profile-content">
        {!! Form::model($auto ,  ['url' => '/update/moto' , 'method' => 'patch' , 'files' => true]) !!}
        <input type="hidden" name="auto_id" value="{{ $auto->id }}" />


        @include('front.autos.user-announcement.motos.form-modif')
        {!! Form::close() !!}
    ...
    <script>
        ...
        $('#checkboxE54').on('change', function(){
            this.value = this.checked ? 1 : 0;
            // alert(this.value);
        }).change();
        $('#checkboxE54x').on('change', function(){
            this.value = this.checked ? 0 : 0;
            // alert(this.value);
        }).change();
    </script>
