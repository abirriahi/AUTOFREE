...
@section('content')
    ...
    <div class="profile-content">
        {!! Form::model($auto ,  ['url' => 'update/camping' , 'method' => 'patch' , 'files' => true]) !!}
        <input type="hidden" name="auto_id" value="{{ $auto->id }}" />


        @include('front.autos.user-announcement.camping.form-modif')
        {!! Form::close() !!}
    ...
    <script>
        ...
        $('#checkboxE61x').on('change', function(){
            this.value = this.checked ? 0 : 0;
            // alert(this.value);
        }).change();
    </script>
    ...
    <script>
        ...
        $('#checkboxE72x').on('change', function(){
            this.value = this.checked ? 0 : 0;
            // alert(this.value);
        }).change();
    </script>
